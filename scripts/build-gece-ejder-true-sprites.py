#!/usr/bin/env python3
"""Gece Ejderi doğru cevap klipleri (5 klip) -> temiz alpha WebP sprite (Buz Ejderi mantığı)."""
...
TRUE_DIR = os.path.join(ROOT, "hero", "dark_dragon", "true")
OUT_DIR = os.path.join(ROOT, "hero", "dark_dragon", "sprite", "true")
MANIFEST_JS = os.path.join(ROOT, "js", "ui", "012yg-nova-gece-ejder-true-manifest.js")
...
CLIP_SPECS = [
    ("dogru_1", "dogru (1).mp4", "gece-ejder-true-dogru-1.webp", 0),
    ("dogru_2", "dogru (2).mp4", "gece-ejder-true-dogru-2.webp", 1),
    ("dogru_3", "dogru (3).mp4", "gece-ejder-true-dogru-3.webp", 2),
    ("dogru_4", "dogru (4).mp4", "gece-ejder-true-dogru-4.webp", 3),
    ("dogru_5", "dogru (5).mp4", "gece-ejder-true-dogru-5.webp", 4),
]
...
def is_gece_foreground(rgb: np.ndarray, key: tuple[int, int, int]) -> np.ndarray:
...
def flood_green(rgba: np.ndarray, key: tuple[int, int, int]) -> np.ndarray:
    out = rgba.copy()
    h, w = out.shape[:2]
    bg = is_greenish(out[:, :, :3], key) & ~is_gece_foreground(out[:, :, :3], key)
...
def scrub_text_green_fringe(rgba: np.ndarray, key: tuple[int, int, int]) -> np.ndarray:
    ...
    fg = is_gece_foreground(out[:, :, :3], key)
    fringe = (
        (out[:, :, 3] > 12)
        & (lum > 68)
        & (gd > 3)
...
def main() -> int:
    ...
    sample_video = resolve_video("dogru (1).mp4")
    ...
    manifest = {
        "version": 3,
        "base": "hero/dark_dragon/sprite/true/",
        "fps": TARGET_FPS,
        "scale": {"sp": 1.0},
        "clips": clips,
    }

    js = "/* AUTO: scripts/build-gece-ejder-true-sprites.py */\n"
    js += "window.NOVA_GECE_EJDER_TRUE_BASE=" + json.dumps(manifest["base"]) + ";\n"
    js += "window.NOVA_GECE_EJDER_TRUE_MANIFEST="
