#!/usr/bin/env python3
"""Taş Muhafız — vitrin, ana ekran, DOĞRU -> seamless WebP sprite sheets."""
...
HERO_DIR = os.path.join(ROOT, "hero", "tas_muhafiz")
OUT_DIR = os.path.join(HERO_DIR, "sprite")
TRUE_DIR = os.path.join(OUT_DIR, "true")
MANIFEST_JS = os.path.join(ROOT, "js", "ui", "012ai-nova-tas-muhafiz-sprite-manifest.js")
TRUE_MANIFEST_JS = os.path.join(ROOT, "js", "ui", "012ak-nova-tas-muhafiz-true-manifest.js")
...
VITRIN_VIDEO = find_video("tas_muhafiz_vitrin.mp4", "*vitrin*.mp4", "*muhafiz*vitrin*")
MAIN_VIDEO = find_video("tas_muhafiz_ana_ekran.mp4", "*ana_ekran*.mp4", "*muhafiz*ana*")
...
def main() -> int:
    ...
    if true_only:
        true_clip = build_true_sheet(
            TRUE_VIDEO,
            os.path.join(TRUE_DIR, "tas-muhafiz-true-dogru.webp"),
        )
        true_root = {
            "version": 1,
            "base": "hero/tas_muhafiz/sprite/true/",
            "scale": {"sp": 1},
            "holdMs": 520,
            "clips": [true_clip],
        }
        write_js(
            ...
            [
                'window.NOVA_TAS_MUHAFIZ_TRUE_BASE="hero/tas_muhafiz/sprite/true/";',
                "window.NOVA_TAS_MUHAFIZ_TRUE_MANIFEST = "
    ...
    if idle_only:
        ...
        if os.path.isfile(manifest_path):
            ...
            combined = {"version": 1, "base": "hero/tas_muhafiz/sprite/", "scale": {"store": 1.12, "detail": 1.5, "main": 1.42}}
    ...
    main_data = build_loop_sheet(
        MAIN_VIDEO,
        os.path.join(OUT_DIR, "tas-muhafiz-main.webp"),
        target_fps=12,
        max_frames=None,
        blend_frames=8,
        target_h=300,
