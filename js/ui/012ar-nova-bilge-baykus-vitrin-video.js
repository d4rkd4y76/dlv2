...
(function () {
  ...
  function scriptBase() {
    ...
    for (i = scripts.length - 1; i >= 0; i--) {
      src = scripts[i].src || '';
      if (src.indexOf('012ar-nova-bilge-baykus') !== -1) {
        return src.replace(/js\/ui\/[^?#]+$/, 'hero/bilge_baykus/sprite/');
      }
    }
    return 'hero/bilge_baykus/sprite/';
  }

  function resolveUrl(base, file) {
    var b = base || 'hero/bilge_baykus/sprite/';
  ...
  function profileManifest(root, profile) {
    ...
    if (profile === 'main' && root.main) {
      return {
        base: root.base || 'hero/bilge_baykus/sprite/',
  ...
  function sheetUrlCandidates(manifest) {
    var file = manifest.sheet || 'bilge-baykus-idle.webp';
    var bases = [];
    if (window.NOVA_BILGE_BAYKUS_SPRITE_BASE) bases.push(window.NOVA_BILGE_BAYKUS_SPRITE_BASE);
    if (manifest.base) bases.push(manifest.base);
    bases.push(scriptBase());
    bases.push('hero/bilge_baykus/sprite/');
    bases.push('./hero/bilge_baykus/sprite/');
    var out = [];
    var seen = {};
  ...
  function getRootManifest() {
    if (window.NOVA_BILGE_BAYKUS_SPRITE_MANIFEST) {
      return Promise.resolve(window.NOVA_BILGE_BAYKUS_SPRITE_MANIFEST);
    }
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-baykus-vitrin-ready', 'nova-hero-baykus-sprite-ready', 'nova-hero-baykus-vitrin-fallback');
  }
  ...
  function startEngine(host, wrap, canvas, manifest, img, opts) {
    ...
    if (eng.dead) return;
    host.classList.add('nova-hero-baykus-vitrin-ready', 'nova-hero-baykus-sprite-ready');
  ...
  function mount(host, opts) {
    if (!host) return null;
    unmount(host);
    host.classList.add('nova-hero-mount--bilge-baykus');
    host.setAttribute('data-baykus-sprite', '1');

    var profile = (opts && opts.profile) || 'store';
    var wrap = document.createElement('div');
    wrap.className = 'nova-hero-baykus-sprite nova-hero-baykus-sprite--' + profile;
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-baykus-sprite__canvas';
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', 'Bilge Baykuş');
    ...
    function tryLoad() {
      ...
      loadAssets(profile, tries > 1).then(function (manifest) {
        ...
        console.error('[baykus sprite] yüklenemedi. Profil:', profile, 'Deneme:', tries, err);
        if (tries < 4) {
          setTimeout(tryLoad, 400 * tries);
          return;
        }
        wrap.classList.add('nova-hero-baykus-sprite--error');
      });
    }
    tryLoad();
    return wrap;
  ...
  window.novaBilgeBaykusMountSprite = mount;
  window.novaBilgeBaykusUnmountSprite = unmount;
  window.novaBilgeBaykusMountVitrinVideo = mount;
  window.novaBilgeBaykusUnmountVitrinVideo = unmount;
  window.novaBilgeBaykusPreloadSprite = function () {
    return Promise.all([
      loadAssets('store', false).catch(function () {}),
      loadAssets('main', false).catch(function () {})
    ]);
  };
  window.novaBilgeBaykusIsSpriteReady = function () {
  ...
  function bootPreload() {
    if (!window.novaSpriteDefer) {
      try { window.novaBilgeBaykusPreloadSprite(); } catch (e) { console.warn('[baykus sprite] preload', e); }
      return;
    }
    window.novaSpriteDefer(function () {
      var hid = window.__novaEquippedHeroId;
      if (hid && hid !== 'bilge_baykus') return;
      try { window.novaBilgeBaykusPreloadSprite(); } catch (e) { console.warn('[baykus sprite] preload', e); }
    }, 3200);
  }
