/* Firtina Okcusu — sprite (mağaza + ana ekran ayrı sheet). */
(function () {
  'use strict';
  ...
  function scriptBase() {
    ...
    for (i = scripts.length - 1; i >= 0; i--) {
      src = scripts[i].src || '';
      if (src.indexOf('012ab-nova-firtina-okcu') !== -1) {
        return src.replace(/js\/ui\/[^?#]+$/, 'hero/firtina_okcusu/sprite/');
      }
    }
    return 'hero/firtina_okcusu/sprite/';
  }

  function resolveUrl(base, file) {
    var b = base || 'hero/firtina_okcusu/sprite/';
  ...
  function profileManifest(root, profile) {
    ...
    if (profile === 'main' && root.main) {
      return {
        base: root.base || 'hero/firtina_okcusu/sprite/',
  ...
  function sheetUrlCandidates(manifest) {
    var file = manifest.sheet || 'firtina-okcu-idle.webp';
    var bases = [];
    if (window.NOVA_FIRTINA_OKCU_SPRITE_BASE) bases.push(window.NOVA_FIRTINA_OKCU_SPRITE_BASE);
    if (manifest.base) bases.push(manifest.base);
    bases.push(scriptBase());
    bases.push('hero/firtina_okcusu/sprite/');
    bases.push('./hero/firtina_okcusu/sprite/');
    var out = [];
    var seen = {};
  ...
  function getRootManifest() {
    if (window.NOVA_FIRTINA_OKCU_SPRITE_MANIFEST) {
      return Promise.resolve(window.NOVA_FIRTINA_OKCU_SPRITE_MANIFEST);
    }
    return Promise.reject(new Error('firtina-manifest-missing'));
  }
  ...
  function loadSheetFromUrls(urls, idx) {
    if (idx >= urls.length) {
      return Promise.reject(new Error('firtina-sheet-all-failed'));
    }
  ...
  function loadAssets(profile, force) {
    ...
    cache.promise = getRootManifest().then(function (root) {
      ...
      if (!manifest || !manifest.sheet) {
        return Promise.reject(new Error('firtina-manifest-profile:' + p));
      }
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-firtina-vitrin-ready', 'nova-hero-firtina-sprite-ready', 'nova-hero-firtina-vitrin-fallback');
  }
  ...
  function startEngine(host, wrap, canvas, manifest, img, opts) {
    ...
    if (eng.dead) return;
    host.classList.add('nova-hero-firtina-vitrin-ready', 'nova-hero-firtina-sprite-ready');
  ...
  function mount(host, opts) {
    if (!host) return null;
    unmount(host);
    host.classList.add('nova-hero-mount--firtina-okcu');
    host.setAttribute('data-firtina-sprite', '1');

    var profile = (opts && opts.profile) || 'store';
    var wrap = document.createElement('div');
    wrap.className = 'nova-hero-firtina-sprite nova-hero-firtina-sprite--' + profile;
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-firtina-sprite__canvas';
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', 'Firtina Okcusu');
    ...
    function tryLoad() {
      ...
      loadAssets(profile, tries > 1).then(function (manifest) {
        ...
        console.error('[firtina sprite] yüklenemedi. Profil:', profile, 'Deneme:', tries, err);
        if (tries < 4) {
          setTimeout(tryLoad, 400 * tries);
          return;
        }
        wrap.classList.add('nova-hero-firtina-sprite--error');
      });
    }
    tryLoad();
    return wrap;
  ...
  window.novaFirtinaOkcuMountSprite = mount;
  window.novaFirtinaOkcuUnmountSprite = unmount;
  window.novaFirtinaOkcuMountVitrinVideo = mount;
  window.novaFirtinaOkcuUnmountVitrinVideo = unmount;
  window.novaFirtinaOkcuPreloadSprite = function () {
    return Promise.all([
      loadAssets('store', false).catch(function () {}),
      loadAssets('main', false).catch(function () {})
    ]);
  };
  window.novaFirtinaOkcuIsSpriteReady = function () {
