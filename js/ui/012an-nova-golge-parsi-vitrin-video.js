/* Golge Parsi — sprite (mağaza + ana ekran ayrı sheet). */
(function () {
  'use strict';
  ...
  function scriptBase() {
    ...
    for (i = scripts.length - 1; i >= 0; i--) {
      src = scripts[i].src || '';
      if (src.indexOf('012an-nova-golge-parsi') !== -1) {
        return src.replace(/js\/ui\/[^?#]+$/, 'hero/golge_parsi/sprite/');
      }
    }
    return 'hero/golge_parsi/sprite/';
  }
  ...
  function profileManifest(root, profile) {
    ...
    if (profile === 'main' && root.main) {
      return {
        base: root.base || 'hero/golge_parsi/sprite/',
  ...
  function sheetUrlCandidates(manifest) {
    var file = manifest.sheet || 'golge-parsi-idle.webp';
    var bases = [];
    if (window.NOVA_GOLGE_PARSI_SPRITE_BASE) bases.push(window.NOVA_GOLGE_PARSI_SPRITE_BASE);
    if (manifest.base) bases.push(manifest.base);
    bases.push(scriptBase());
    bases.push('hero/golge_parsi/sprite/');
    bases.push('./hero/golge_parsi/sprite/');
    var out = [];
    var seen = {};
  ...
  function getRootManifest() {
    if (window.NOVA_GOLGE_PARSI_SPRITE_MANIFEST) {
      return Promise.resolve(window.NOVA_GOLGE_PARSI_SPRITE_MANIFEST);
    }
    return Promise.reject(new Error('golge-manifest-missing'));
  }
  ...
  function loadSheetFromUrls(urls, idx) {
    if (idx >= urls.length) {
      return Promise.reject(new Error('golge-sheet-all-failed'));
    }
  ...
  function loadAssets(profile, force) {
    ...
    cache.promise = getRootManifest().then(function (root) {
      ...
      if (!manifest || !manifest.sheet) {
        return Promise.reject(new Error('golge-manifest-profile:' + p));
      }
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-golge-vitrin-ready', 'nova-hero-golge-sprite-ready', 'nova-hero-golge-vitrin-fallback');
  }
  ...
  function startEngine(host, wrap, canvas, manifest, img, opts) {
    ...
    if (eng.dead) return;
    host.classList.add('nova-hero-golge-vitrin-ready', 'nova-hero-golge-sprite-ready');
  ...
  function mount(host, opts) {
    if (!host) return null;
    unmount(host);
    host.classList.add('nova-hero-mount--golge-parsi');
    host.setAttribute('data-golge-sprite', '1');

    var profile = (opts && opts.profile) || 'store';
    var wrap = document.createElement('div');
    wrap.className = 'nova-hero-golge-sprite nova-hero-golge-sprite--' + profile;
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-golge-sprite__canvas';
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', 'Gölge Parsı');
    ...
    function tryLoad() {
      ...
      loadAssets(profile, tries > 1).then(function (manifest) {
        ...
        console.error('[golge sprite] yüklenemedi. Profil:', profile, 'Deneme:', tries, err);
        if (tries < 4) {
          setTimeout(tryLoad, 400 * tries);
          return;
        }
        wrap.classList.add('nova-hero-golge-sprite--error');
      });
    }
    tryLoad();
    return wrap;
  ...
  window.novaGolgeParsiMountSprite = mount;
  window.novaGolgeParsiUnmountSprite = unmount;
  window.novaGolgeParsiMountVitrinVideo = mount;
  window.novaGolgeParsiUnmountVitrinVideo = unmount;
  window.novaGolgeParsiPreloadSprite = function () {
    return Promise.all([
      loadAssets('store', false).catch(function () {}),
      loadAssets('main', false).catch(function () {})
    ]);
  };
  window.novaGolgeParsiIsSpriteReady = function () {
  ...
  function bootPreload() {
    if (!window.novaSpriteDefer) {
      try { window.novaGolgeParsiPreloadSprite(); } catch (e) { console.warn('[golge sprite] preload', e); }
      return;
    }
    window.novaSpriteDefer(function () {
      var hid = window.__novaEquippedHeroId;
      if (hid && hid !== 'golge_parsi') return;
      try { window.novaGolgeParsiPreloadSprite(); } catch (e) { console.warn('[golge sprite] preload', e); }
    }, 3200);
  }
