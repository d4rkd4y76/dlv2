...
(function () {
  ...
  function scriptBase() {
    ...
    for (i = scripts.length - 1; i >= 0; i--) {
      src = scripts[i].src || '';
      if (src.indexOf('012v-nova-alev-ejder') !== -1) {
        return src.replace(/js\/ui\/[^?#]+$/, 'hero/flame_dragon/sprite/');
      }
    }
    return 'hero/flame_dragon/sprite/';
  }
  ...
  function profileManifest(root, profile) {
    ...
    if (profile === 'main' && root.main) {
      return {
        base: root.base || 'hero/flame_dragon/sprite/',
  ...
  function sheetUrlCandidates(manifest) {
    var file = manifest.sheet || 'alev-ejder-idle.webp';
    var bases = [];
    if (window.NOVA_ALEV_EJDER_SPRITE_BASE) bases.push(window.NOVA_ALEV_EJDER_SPRITE_BASE);
    if (manifest.base) bases.push(manifest.base);
    bases.push(scriptBase());
    bases.push('hero/flame_dragon/sprite/');
    bases.push('./hero/flame_dragon/sprite/');
    var out = [];
    var seen = {};
  ...
  function getRootManifest() {
    if (window.NOVA_ALEV_EJDER_SPRITE_MANIFEST) {
      return Promise.resolve(window.NOVA_ALEV_EJDER_SPRITE_MANIFEST);
    }
    return Promise.reject(new Error('alev-manifest-missing'));
  }
  ...
  function loadSheetFromUrls(urls, idx) {
    if (idx >= urls.length) {
      return Promise.reject(new Error('alev-sheet-all-failed'));
    }
  ...
  function loadAssets(profile, force) {
    ...
    cache.promise = getRootManifest().then(function (root) {
      ...
      if (!manifest || !manifest.sheet) {
        return Promise.reject(new Error('alev-manifest-profile:' + p));
      }
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-alev-vitrin-ready', 'nova-hero-alev-sprite-ready', 'nova-hero-alev-vitrin-fallback');
  }
  ...
  function startEngine(host, wrap, canvas, manifest, img, opts) {
    ...
    if (eng.dead) return;
    host.classList.add('nova-hero-alev-vitrin-ready', 'nova-hero-alev-sprite-ready');
  ...
  function mount(host, opts) {
    ...
    canvas.className = 'nova-hero-alev-sprite__canvas';
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', 'Alev Ejderi');
  ...
  window.novaAlevEjderMountSprite = mount;
  window.novaAlevEjderUnmountSprite = unmount;
  window.novaAlevEjderMountVitrinVideo = mount;
  window.novaAlevEjderUnmountVitrinVideo = unmount;
  window.novaAlevEjderPreloadSprite = function () {
    return Promise.all([
      loadAssets('store', false).catch(function () {}),
      loadAssets('main', false).catch(function () {})
    ]);
  };
  window.novaAlevEjderIsSpriteReady = function () {
