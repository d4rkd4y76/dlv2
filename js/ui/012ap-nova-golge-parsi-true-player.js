/* Golge Parsi — tek kişilik doğru cevap sprite klipleri */
(function () {
  'use strict';
  ...
  function rootManifest() {
    return window.NOVA_GOLGE_PARSI_TRUE_MANIFEST || null;
  }

  function scriptBase() {
    if (window.NOVA_GOLGE_PARSI_TRUE_BASE) return window.NOVA_GOLGE_PARSI_TRUE_BASE;
    return 'hero/golge_parsi/sprite/true/';
  }
  ...
  function isTasMuhafizEquipped() {
    return getEquippedHeroId() === 'golge_parsi';
  }
  ...
  TrueClipEngine.prototype.loop = function (now) {
    ...
    if (this.done) {
      ...
      this.raf = 0;
      if (this.wrap) this.wrap.classList.add('nova-hero-golge-true-wrap--done');
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-golge-true-ready', 'nova-sp-fx-true-sprite', 'nova-hero-mount--golge-parsi');
  }
  ...
  function mountAndPlay(host, assets) {
    ...
    wrap.className = 'nova-hero-golge-true-wrap';
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-golge-true-sprite__canvas';
    canvas.setAttribute('aria-hidden', 'true');
    wrap.appendChild(canvas);
    host.appendChild(wrap);
    host.classList.add('nova-hero-golge-true-ready');
  ...
  function playTrueClip(host, routine) {
    ...
    unmount(host);
    host.classList.add('nova-sp-fx-true-sprite', 'nova-hero-mount--golge-parsi');
  ...
  window.novaGolgeParsiHasTrueClips = hasTrueClips;
  window.novaGolgeParsiPlayTrueClip = playTrueClip;
  window.novaGolgeParsiTrueUnmount = unmount;
  window.novaGolgeParsiPreloadTrueClips = preloadTrueClips;
  window.novaGolgeParsiPreloadTrueClipsIfEquipped = preloadTrueClipsIfEquipped;
  window.novaGolgeParsiEnsureTrueClipsReady = preloadAllTrueClips;
  window.novaGolgeParsiPickTrueClipRoutine = pickTrueClipRoutine;

  function playSpFx(host, variant, routine) {
    return playTrueClip(host, typeof routine === 'number' ? routine : 0);
  }
  window.novaGolgeParsiPlaySpFx = playSpFx;
})();
