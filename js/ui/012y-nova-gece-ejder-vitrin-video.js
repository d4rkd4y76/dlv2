/* Gece Ejderi — sprite (mağaza + ana ekran ayrı sheet). */
(function () {
  'use strict';
  ...
  function scriptBase() {
    ...
    for (i = scripts.length - 1; i >= 0; i--) {
      src = scripts[i].src || '';
      if (src.indexOf('012y-nova-gece-ejder') !== -1) {
        return src.replace(/js\/ui\/[^?#]+$/, 'hero/dark_dragon/sprite/');
      }
    }
    return 'hero/dark_dragon/sprite/';
  }

  function resolveUrl(base, file) {
    var b = base || 'hero/dark_dragon/sprite/';
  ...
  function profileManifest(root, profile) {
    ...
    if (profile === 'main' && root.main) {
      return {
        base: root.base || 'hero/dark_dragon/sprite/',
  ...
  function sheetUrlCandidates(manifest) {
    var file = manifest.sheet || 'gece-ejder-idle.webp';
    var bases = [];
    if (window.NOVA_GECE_EJDER_SPRITE_BASE) bases.push(window.NOVA_GECE_EJDER_SPRITE_BASE);
    if (manifest.base) bases.push(manifest.base);
    bases.push(scriptBase());
    bases.push('hero/dark_dragon/sprite/');
    bases.push('./hero/dark_dragon/sprite/');
    var out = [];
    var seen = {};
  ...
  function getRootManifest() {
    if (window.NOVA_GECE_EJDER_SPRITE_MANIFEST) {
      return Promise.resolve(window.NOVA_GECE_EJDER_SPRITE_MANIFEST);
    }
    return Promise.reject(new Error('gece-manifest-missing'));
  }
  ...
  function loadSheetFromUrls(urls, idx) {
    if (idx >= urls.length) {
      return Promise.reject(new Error('gece-sheet-all-failed'));
    }
  ...
  function loadAssets(profile, force) {
    ...
    cache.promise = getRootManifest().then(function (root) {
      ...
      if (!manifest || !manifest.sheet) {
        return Promise.reject(new Error('gece-manifest-profile:' + p));
      }
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-gece-vitrin-ready', 'nova-hero-gece-sprite-ready', 'nova-hero-gece-vitrin-fallback');
  }
  ...
  function startEngine(host, wrap, canvas, manifest, img, opts) {
    ...
    if (eng.dead) return;
    host.classList.add('nova-hero-gece-vitrin-ready', 'nova-hero-gece-sprite-ready');
  ...
  function mount(host, opts) {
    if (!host) return null;
    unmount(host);
    host.classList.add('nova-hero-mount--gece-ejder');
    host.setAttribute('data-gece-sprite', '1');

    var profile = (opts && opts.profile) || 'store';
    var wrap = document.createElement('div');
    wrap.className = 'nova-hero-gece-sprite nova-hero-gece-sprite--' + profile;
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-gece-sprite__canvas';
    canvas.setAttribute('role', 'img');
    canvas.setAttribute('aria-label', 'Gece Ejderi');
    ...
    function tryLoad() {
      ...
      loadAssets(profile, tries > 1).then(function (manifest) {
        ...
        console.error('[gece sprite] yüklenemedi. Profil:', profile, 'Deneme:', tries, err);
        if (tries < 4) {
          setTimeout(tryLoad, 400 * tries);
          return;
        }
        wrap.classList.add('nova-hero-gece-sprite--error');
      });
    }
    tryLoad();
    return wrap;
  ...
  window.novaGeceEjderMountSprite = mount;
  window.novaGeceEjderUnmountSprite = unmount;
  window.novaGeceEjderMountVitrinVideo = mount;
  window.novaGeceEjderUnmountVitrinVideo = unmount;
  window.novaGeceEjderPreloadSprite = function () {
    return Promise.all([
      loadAssets('store', false).catch(function () {}),
      loadAssets('main', false).catch(function () {})
    ]);
  };
  window.novaGeceEjderIsSpriteReady = function () {
