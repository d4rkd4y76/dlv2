...
(function () {
  ...
  function rootManifest() {
    return window.NOVA_BILGE_BAYKUS_TRUE_MANIFEST || null;
  }

  function scriptBase() {
    if (window.NOVA_BILGE_BAYKUS_TRUE_BASE) return window.NOVA_BILGE_BAYKUS_TRUE_BASE;
    return 'hero/bilge_baykus/sprite/true/';
  }
  ...
  function isBilgeBaykusEquipped() {
    return getEquippedHeroId() === 'bilge_baykus';
  }
  ...
  function preloadTrueClipsIfEquipped(force) {
    if (!isBilgeBaykusEquipped() || !hasTrueClips()) return Promise.resolve(false);
  ...
  TrueClipEngine.prototype.loop = function (now) {
    ...
    if (this.done) {
      ...
      this.raf = 0;
      if (this.wrap) this.wrap.classList.add('nova-hero-baykus-true-wrap--done');
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-baykus-true-ready', 'nova-sp-fx-true-sprite', 'nova-hero-mount--bilge-baykus');
  }
  ...
  function mountAndPlay(host, assets) {
    ...
    wrap.className = 'nova-hero-baykus-true-wrap';
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-baykus-true-sprite__canvas';
    canvas.setAttribute('aria-hidden', 'true');
    wrap.appendChild(canvas);
    host.appendChild(wrap);
    host.classList.add('nova-hero-baykus-true-ready');
  ...
  function playTrueClip(host, routine) {
    ...
    unmount(host);
    host.classList.add('nova-sp-fx-true-sprite', 'nova-hero-mount--bilge-baykus');
  ...
  window.novaBilgeBaykusHasTrueClips = hasTrueClips;
  window.novaBilgeBaykusPlayTrueClip = playTrueClip;
  window.novaBilgeBaykusTrueUnmount = unmount;
  window.novaBilgeBaykusPreloadTrueClips = preloadTrueClips;
  window.novaBilgeBaykusPreloadTrueClipsIfEquipped = preloadTrueClipsIfEquipped;
  window.novaBilgeBaykusEnsureTrueClipsReady = preloadAllTrueClips;
  window.novaBilgeBaykusPickTrueClipRoutine = pickTrueClipRoutine;

  function playSpFx(host, variant, routine) {
    return playTrueClip(host, typeof routine === 'number' ? routine : 0);
  }
  window.novaBilgeBaykusPlaySpFx = playSpFx;
})();
