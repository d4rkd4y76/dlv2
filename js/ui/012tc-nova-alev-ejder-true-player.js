...
(function () {
  ...
  function rootManifest() {
    return window.NOVA_ALEV_EJDER_TRUE_MANIFEST || null;
  }

  function scriptBase() {
    if (window.NOVA_ALEV_EJDER_TRUE_BASE) return window.NOVA_ALEV_EJDER_TRUE_BASE;
    return 'hero/flame_dragon/sprite/true/';
  }
  ...
  function isAlevEquipped() {
    return getEquippedHeroId() === 'alev_ejder';
  }
  ...
  function preloadTrueClipsIfEquipped(force) {
    if (!isAlevEquipped() || !hasTrueClips()) return Promise.resolve(false);
  ...
  function unmount(host) {
    ...
    host.innerHTML = '';
    host.classList.remove('nova-hero-alev-true-ready', 'nova-sp-fx-true-sprite', 'nova-hero-mount--alev-ejder');
  }
  ...
  function mountAndPlay(host, assets) {
    ...
    wrap.className = 'nova-hero-alev-true-wrap';
    var canvas = document.createElement('canvas');
    canvas.className = 'nova-hero-alev-true-sprite__canvas';
    canvas.setAttribute('aria-hidden', 'true');
    wrap.appendChild(canvas);
    host.appendChild(wrap);
    host.classList.add('nova-hero-alev-true-ready');
  ...
  function playTrueClip(host, routine) {
    ...
    unmount(host);
    host.classList.add('nova-sp-fx-true-sprite', 'nova-hero-mount--alev-ejder');
    ...
    return preloadAllTrueClips().then(function () {
      ...
      console.warn('alev true clip', err);
      return waitMs(300);
    });
  }
  ...
  window.novaAlevEjderHasTrueClips = hasTrueClips;
  window.novaAlevEjderPlayTrueClip = playTrueClip;
  window.novaAlevEjderTrueUnmount = unmount;
  window.novaAlevEjderPreloadTrueClips = preloadTrueClips;
  window.novaAlevEjderPreloadTrueClipsIfEquipped = preloadTrueClipsIfEquipped;
  window.novaAlevEjderEnsureTrueClipsReady = preloadAllTrueClips;
  window.novaAlevEjderPickTrueClipRoutine = pickTrueClipRoutine;
})();
